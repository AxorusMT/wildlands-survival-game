...
/** Mirages wear the look of a real foe; only the shaded eye can tell. */
export const MIRAGE_GUISE: Record<string, string> = {
  mirage: 'sand_raptor',
...
export function drawAnimal(
  ...
  const m = track(a.id, a.x, a.y, a.hp, t),
    ...
    key = a.type === 'boss' ? 'boss' + g.s.altar.level : (MIRAGE_GUISE[a.type] ?? a.type);
  const sy = Math.round(y + (m.sy - a.y) / PX);
  // A mirage shimmers faintly; to shaded eyes it is barely there.
